module ImageProcessingHelper
  ...
  def self.create_thumbnail(image_path, thumbnail_directory)
    ...
    `convert -thumbnail 180x180 -gravity center -background white -extent 180x180 #{image_path} #{thumbnail_directory}/#{File.basename(image_path).split('.').first}.png`
    $?.to_i
  end
